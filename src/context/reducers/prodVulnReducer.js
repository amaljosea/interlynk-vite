const prodVulnReducer = (state, action) => {
  ...
  switch (type) {
    ...
    case 'ON_CHANGE_JUSTIFICATION':
      return { ...state, justification: payload?.value, justifyName: payload?.name }
    ...
    case 'DECREMENT_PAGE':
      return { ...state, pageIndex: state.pageIndex !== 0 && state.pageIndex - 1, before: payload }
    case 'INCREMENT_PAGE':
      const { total, after } = payload
      return { ...state, pageIndex: state.pageIndex < Math.ceil(total) && state.pageIndex + 1, after: after }
    case 'SET_SORT_ORDER':
      return { ...state, field: payload.field, direction: payload.direction, pageIndex: 1 }
    ...
    case 'FILTER_COMPONENT':
      return { ...state, components: [...payload]?.includes('all') ? [] : payload, pageIndex: 1, after: '', before: '' }
    ...
    case 'FILTER_EPSS':
      return { ...state, epss: payload, pageIndex: 1, after: '', before: '', minEpss: 0, maxEpss: 0 }
    case 'FILTER_DIRECT':
