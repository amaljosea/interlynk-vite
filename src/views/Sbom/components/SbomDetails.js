import { Badge, Flex, HStack, Icon, IconButton, Stack, Tag, TagLabel, Text, Tooltip, useDisclosure } from '@chakra-ui/react'
...
const SbomDetails = ({ sbom, getCompData, getVulnData }) => {
  ...
  const [getPrimaryComp, { data: primaryComp }] = useLazyQuery(signedUrlParams ? GetSharPrimartComp : GetPrimaryComponent,{ fetchPolicy: 'network-only' })

  const handleRelationView = async () => {
    await getPrimaryComp({ variables: { projectId: signedUrlParams ? undefined : productId, sbomId: sbomId, primary: true, field, direction }
  ...
  const onFilterVuln = (value) => {
    ...
    getVulnData({ projectId: signedUrlParams ? undefined : productId, sbomId: sbomId, source: undefined, severity: value || undefined, first: totalRows, last: undefined, after: undefined, before: undefined, field: field, direction: direction }).then((res) => {
      if (res.data) {
        if (signedUrlParams) {
  ...
  return (
    <>
      <Flex direction={'row'} alignItems={'flex-start'} gap={5} width={'100%'}>
        <Icon as={FaCubes} h={'64px'} w={'64px'} color='blue.300' onClick={handleRelationView} cursor={'pointer'} />
        <Flex direction={'column'} gap={0.5}>
          ...
          <Text fontSize={'sm'} my={1}>{primaryComponent?.description}</Text>
          {/* SCAN STATUS */}
          {!signedUrlParams && (
      ...
      {isOpen && primaryComp && <GraphDrawer isOpen={isOpen} onClose={onClose} primaryComp={signedUrlParams ? primaryComp?.shareLynkQuery?.sbom?.components : primaryComp?.sbom?.components} activeComp={null} />}
    </>
  )
}
