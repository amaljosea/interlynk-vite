...
import { Box, Button, Drawer, DrawerBody, DrawerCloseButton, DrawerContent, DrawerHeader, DrawerOverlay, DrawerFooter, Flex, FormControl, FormLabel, Select, Stack, Table, Tag, TagCloseButton, TagLabel, Tbody, Td, Text, Th, Thead, Tooltip, Tr, FormErrorMessage, FormErrorIcon, useDisclosure, } from '@chakra-ui/react'
...
const RelationshipDrawer = ({ isOpen, onClose, data, total, fetchCompData, compPath }) => {
  ...
  const { data: allComponents } = useQuery(GetAllComponents, {
    ...
    variables: { projectId: productId, sbomId: sbomId, first: total, field: 'COMPONENTS_UPDATED_AT', direction: 'DESC' },
    onCompleted: (data) => {
      if (data) {
        setRelation('')
        setComponent('')
    ...
    await removeRelation({
      ...
      .then((res) => {
        if (res.data) {
          ...
          const filterData = dependsOnList.filter((item) => item.id !== activeComp.id)
  ...
  return (
    <Drawer size='lg' isOpen={isOpen} placement='right' onClose={onClose} closeOnOverlayClick={true}>
      <DrawerOverlay />
      <DrawerContent>
        <DrawerCloseButton />
        <DrawerHeader borderBottomWidth='1px' color='gray.600'>Relationships</DrawerHeader>
        <DrawerBody>
          <Card px={0} mx={0}>
            <CardHeader>
              <Flex width='100%' direction={'row'} alignItems={'center'} justifyContent={'flex-start'} wrap={'wrap'} gap={2} >
                <Text fontWeight={'medium'}>{name}</Text>
                <Tag colorScheme='blue'>{version}</Tag>
              </Flex>
            </CardHeader>
            <CardBody>
              <Flex flexDir={'column'} alignItems={'flex-start'} width={'100%'} gap={4} >
                {/* CREATE RELATIONSHIP */}
                <Stack width={'100%'} direction={'column'} alignItems={'flex-start'} gap={2} mt={6} >
                  <FormControl>
                  ...
                  {allComponents && (
                    <FormControl isInvalid={list.length > 0}>
                      ...
                      <Select id='component' size='sm' value={component} onChange={(e) => setComponent(e.target.value)} >
                        <option value=''>-- Select --</option>
                  ...
                  <Button size='md' mt={2} width={'fit-content'} colorScheme='blue' onClick={handleAdd} isDisabled={ relation === '' || component === '' || list.length > 0 }>
                    Add
                  </Button>
                </Stack>
                  ...
                  <Thead>
                    <Tr>
                      {['Type', 'Component'].map((item, index) => (
                        <Th key={index} pl={0} width={'100px'}><Box>{item}</Box></Th>
                      ))}
                    </Tr>
                  </Thead>
                  <Tbody>
                    ...
                    <Tr>
                      <Td pl={0} width={'120px'}>
                        <Text fontSize='xs' fontWeight={'medium'}>Dependency Of</Text>
                      </Td>
                      <Td pl={0} width={'300px'}>
                        <Flex flexDirection={'row'} flexWrap={'wrap'} gap={2}>
                          {dependencyOfList.map((comp, index) => (
                            <Tag size={'sm'} key={index} variant='subtle' colorScheme={'blue'} width={'fit-content'} >
                    ...
                    <Tr>
                      <Td pl={0} width={'120px'}>
                        <Text fontSize='xs' fontWeight={'medium'}>Depends On</Text>
                      </Td>
                      <Td pl={0} width={'300px'}>
                        <Flex flexDirection={'row'} flexWrap={'wrap'} gap={2}>
                          {[...dependsOnList]
                ...
                {isDelOpen && activeComp && (
                  <RelDeleteModal isOpen={isOpen} onClose={onClose} handleRemove={handleRemove} activeComp={activeComp} />
                )}

                {/* PATHS */}
                <Text fontSize={'lg'} fontWeight={'medium'} mt={6}>Tree View</Text>
                {compPath?.length > 0 ? (
                  <Stack width={'100%'} mt={10} dir='column' spacing={2} alignItems={'center'} justifyContent={'center'} >
                    {shortestPath.path?.length > 0 ? (
                      shortestPath.path.map((item, index) => (
                        <>
                          <Tag key={item.id} size='sm' colorScheme={ index === 0 || index === shortestPath.path.length - 1 ? 'blue' : 'green' }>
                      ...
                      <Text fontSize={'sm'}>Component is not connected to Primary component</Text>
                    )}
                  </Stack>
                ) : (
        ...
        <DrawerFooter>
          <Button variant='solid' colorScheme='blue' onClick={handleSave}>Done</Button>
        </DrawerFooter>
      </DrawerContent>
    </Drawer>
  )
